...
def preprocess_data(data, sequence_length=60, train_size=0.8, scale=True):
    ...
    feature_columns = [
        ...
        'Volume_Change', 'OBV', 'MACD', 'MACD_Signal', 'BB_Upper', 'BB_Middle', 'BB_Lower', 'VIX'
    ]
...
class EnhancedDeepResNet(nn.Module):
    def __init__(self, input_dim, hidden_dims=[128, 256, 512], dropout_rate=0.3):
        super(EnhancedDeepResNet, self).__init__()
        self.input_dim = input_dim

        # LSTM
        self.lstm = nn.LSTM(input_size=input_dim, hidden_size=hidden_dims[0], num_layers=3,
        ...
        combined_dim = hidden_dims[0] * 2 + hidden_dims[0] + hidden_dims[0]  # LSTM (256) + TCN (128) + Transformer (128)
        self.fc1 = nn.Linear(combined_dim, hidden_dims[1])
        self.fc2 = nn.Linear(hidden_dims[1], hidden_dims[2])
        self.residual = nn.Linear(combined_dim, hidden_dims[2])
    ...
    def forward(self, x):
        ...
        combined = torch.cat((lstm_out, tcn_out, transformer_out), dim=1)
...
# --- Training Functions ---
def train_model(model, X_train, y_train, X_val, y_val, device, num_epochs=200, batch_size=64, patience=30):
...
def train_ensemble_model(X_train, y_train, X_val, y_val, input_dim, device, ensemble_size=10):
    models, val_losses = [], []
    ...
    weights = np.exp(-np.array(val_losses))  # Exponential weighting based on loss
    weights /= weights.sum()
...
def generate_future_predictions(models, weights, original_data, feature_columns, scaler, device, days=30):
    ...
    for _ in range(days):
        ...
        pred_scaled = ensemble_predict(models, weights, input_tensor, device)[0]  # [Open, High, Low, Close, Volume]

        pred_array = np.zeros((1, len(feature_columns)))
        pred_array[0, :5] = pred_scaled
        pred_values = scaler.inverse_transform(pred_array)[0, :5]  # Unscale Open, High, Low, Close, Volume
        ...
        future_predictions.append(pred_values[3])  # Store Close price
...
def plot_future_predictions(dates, future_predictions, historical_data, stock_symbol):
    ...
    plt.title(f"{stock_symbol} Future Price Prediction (30 Days)", fontsize=14)
    plt.xlabel('Date', fontsize=12)
    plt.ylabel('Price ($)', fontsize=12)
    plt.legend(fontsize=10)
    plt.grid(True, linestyle='--', alpha=0.7)
...
def main():
    try:
        ...
        models, weights = train_ensemble_model(
            X_train, y_train, X_val, y_val, input_dim=X_train.shape[2], device=device, ensemble_size=10
        )
